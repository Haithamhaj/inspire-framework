 ...
 *
 * Uses a unique disposable account per run and deletes it via admin endpoint
 * if ADMIN_PASSWORD is available in env or .env.local.
 */
...
async function main() {
  ...
  const discountCode = "HAITHAM";

  const webPages = {};
  for (const path of ["/", "/pricing", "/assess", "/login"]) {
    const r = await fetchText(path);
    webPages[path] = r.status;
  }

  // Register (treat 409 as ok)
  {
    const r = await fetchJson(`${apiBase}/auth/register`, {
      method: "POST",
      body: { name: "Smoke Test", email, password, job_title: "QA", consent_given: true },
    });
    assert(r.status === 201 || r.status === 409, `register failed: ${r.status}`);
  }

  // Login
  const login = await fetchJson(`${apiBase}/auth/login`, { method: "POST", body: { email, password } });
  ...
  const answers = questions.json.questions.map((q) => ({ questionId: q.questionId, optionId: q.options?.[0]?.optionId }));
  ...
  const submit = await fetchJson(`${apiBase}/assessments/${assessmentId}/submit`, {
    method: "POST",
    headers: authHeaders,
    body: { answers, open_answer: "Automated production smoke test.", completion_time_seconds: 180, payment_id: paymentId },
  });
