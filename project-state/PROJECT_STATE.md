...
- Lemon Squeezy checkout is forced to live mode in code for the current live product, so stale or hidden `LEMON_SQUEEZY_TEST_MODE` environment values cannot make checkout creation use the wrong Lemon environment.
