...
function TitleBar({ label, glow = false }: { label: string; glow?: boolean }) {
  return (
    <div
      className="flex items-center gap-2 px-5 py-3 border-b border-white/8 flex-shrink-0"
...
export function InspireExplainer() {
  ...
  /* fixed 1280×720 canvas — scale to fit whatever the browser viewport is */
  return (
    <div className="w-screen h-screen flex items-center justify-center overflow-hidden" style={{ background: "#060610" }}>
      {/* Fixed-size inner canvas: always 1280×720 */}
      <div
        dir={dir}
        className="relative overflow-hidden select-none flex flex-col"
        style={{ width: 1280, height: 720, background: "linear-gradient(140deg,#08080f 0%,#0d0820 55%,#08080f 100%)", flexShrink: 0 }}
      >
        {/* ── Ambient glows ── */}
        ...
        <div className="relative z-10 flex items-center justify-between px-10 pt-7 pb-0 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/25 text-xs font-mono">
              {scene === 1 ? "no profile active" : scene === 2 ? "building profile…" : "profile active"}
            </span>
          </div>
        </div>
            ...
            {scene === 3 && (
              <motion.div key={`s3-${loopKey}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.4 }} className="flex flex-col gap-5">
                <Glass
                  ...
                  style={{ borderColor: "rgba(225,29,72,.3)", boxShadow: "0 0 80px rgba(225,29,72,.12), 0 0 160px rgba(124,58,237,.07), inset 0 1px 0 rgba(255,255,255,.08)" }}
                >
                ...
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.7, duration: .5 }} className="flex items-center justify-between px-1">
                  ...
                  <div className="flex items-center gap-5">
                    <span className="text-white/45 text-sm font-black tracking-[.22em]">{c.logo}</span>
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      className="px-7 py-3 rounded-full text-sm font-bold text-white"
                      style={{ background: "linear-gradient(135deg,#e11d48,#7c3aed)", boxShadow: "0 0 28px rgba(225,29,72,.5), 0 4px 14px rgba(0,0,0,.3)" }}
                    >
                      {c.cta}
                    </motion.button>
                  </div>
